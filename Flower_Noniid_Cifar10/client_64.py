...
def load_dataset():
    ...
    for image, label in train_set:
        class_subsets[label].append(image)
    selected_label = random.randint(0, 9)
    selected_subset = class_subsets[selected_label]
    num_samples_selected = int(0.6 * len(selected_subset))
    selected_images = random.sample(selected_subset, num_samples_selected)
    remaining_images = []
    for label, subset in enumerate(class_subsets):
        if label != selected_label:
            num_samples_remaining = int(0.4 * len(subset) / 9)  # Uniform sampling from other labels
            remaining_images.extend(random.sample(subset, num_samples_remaining))

    selected_images.extend(remaining_images)

    labels = [selected_label] * num_samples_selected + [label for label in range(10) if label != selected_label] * num_samples_remaining

    train_dataset = list(zip(selected_images, labels))
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        ...
        print("parametrs shape",params.shape)
